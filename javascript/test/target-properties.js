...
describe('Target Properties', () => {
  it('Gets a property', () => {
    const t = { foo: 'bar' };
    dakka.run('var a = [foo]; return a;', null, t, (val) => {
  ...
  it('Sets a property with modifier', () => {
    const t = { foo: 2 };
    dakka.run("[foo] += 5; [foo] -= 1; [foo] /= 2; [foo] *= 3; return [foo] %= 4;", null, t, () => {
  ...
  it('Throws on undefined properties', () => {
    const t = {};
    assert.throws(() => { dakka.run('[foo] = 1;', null, t) });
    assert.throws(() => { dakka.run('var a = [foo];', null, t) });
  });
});
